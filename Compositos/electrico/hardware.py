...
class Keithley224:
    ...
    def conectar(self, rm):
        ...
        self.inst.write("R0X") # Auto-rango apagado (RAM)
        self.standby()
...
class Agilent34420A:
    ...
    def conectar(self, rm):
        ...
        self.inst.write("*RST") # Resetea memoria volátil
    ...
    def leer_voltaje(self):
        ...
        try:
            ...
            raise RuntimeError(f"Fallo de lectura 34420A: {e}")
        ...
        try:
            ...
            while self.corriendo:
                ...
                if not self.pausado_cbias:
                    ...
                    r1_inst = float('nan') if corriente_objetivo == 0 else (v1_inst / (corriente_objetivo * 1e-3))
                    r2_inst = float('nan') if (corriente_objetivo == 0 or not usa_ch2) else (v2_inst / (corriente_objetivo * 1e-3))
                ...
                if num_bias > 0:
                    for _ in range(num_bias):
                        ...
                        r1_bias = float('nan') if self.estado['corr_bias'] == 0 else (v1_bias / (self.estado['corr_bias'] * 1e-3))
                        r2_bias = float('nan') if (self.estado['corr_bias'] == 0 or not usa_ch2) else (v2_bias / (self.estado['corr_bias'] * 1e-3))
                        
                        t_min = (time.time() - tiempo_inicio) / 60.0
                        self.datos_actualizados.emit(t_min, v1_bias, r1_bias, v2_bias, r2_bias, self.estado['corr_bias'], True)
                        
                        fila = [
                            f"{t_min:.4f}", nan, self.estado['corr_bias'],
                            nan, nan, r1_bias, v1_bias,
                            nan, nan, r2_bias, v2_bias,
                            self.estado['ancho_pulso'], self.estado['periodo_pulso'], 
                            self.estado['limite_voltaje'], self.estado['rango'], 
                            nplc, usa_ch2, self.estado['num_mediciones_bias']
                        ]
